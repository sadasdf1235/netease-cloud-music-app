...
import type { 
  ...
  HotPlaylistCategoriesResponse,
  PlaylistCategoriesResponse,
 ...
 * 获取热门歌单分类
 * @param useCache 是否使用缓存，默认为true
 * @returns Promise<HotPlaylistCategoriesResponse> 热门歌单分类数据
 */
export function getHotPlaylistCategories(useCache = true) {
  const apiCall = () => get<HotPlaylistCategoriesResponse>('/playlist/hot');
  return useCache
 ...
 * 获取所有歌单分类
 * @param useCache 是否使用缓存，默认为true
 * @returns Promise<PlaylistCategoriesResponse> 歌单分类数据
 */
export function getPlaylistCategories(useCache = true) {
  const apiCall = () => get<PlaylistCategoriesResponse>('/playlist/catlist');
  return useCache
 ...
 * @param offset 偏移数量，用于分页
 * @param useCache 是否使用缓存，默认为true
 * @returns Promise<TopPlaylistsResponse> 歌单列表数据
 */
export function getTopPlaylists(cat = '全部', limit = 30, offset = 0, useCache = true) {
  const apiCall = () => get<TopPlaylistsResponse>('/top/playlist', { cat, limit, offset });
  return useCache
 ...
 * @returns Promise<SubscribePlaylistResponse> 收藏结果
 */
export function subscribePlaylist(id: number, t: 1 | 2) {
  return get<SubscribePlaylistResponse>('/playlist/subscribe', { id, t });
}
 ...
 * @param limit 取出数量，默认为30
 * @param offset 偏移数量，用于分页
 * @returns Promise<UserPlaylistsResponse> 用户歌单数据
 */
export function getUserPlaylists(uid: number, limit = 30, offset = 0) {
  return get<UserPlaylistsResponse>('/user/playlist', { uid, limit, offset });
}
 ...
 * @returns Promise<PlaylistCommentsResponse> 歌单评论数据
 */
