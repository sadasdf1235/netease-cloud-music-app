  ...
  <div class="artist-detail-page">
    ...
    <div class="flex flex-col md:flex-row gap-6 mb-6">
      <div class="relative w-64 h-64 flex-shrink-0 rounded-lg overflow-hidden shadow-md">
        ...
        <template v-else>
          <img :src="artistDetail.artist?.picUrl + '?param=512y512'" class="w-full h-full object-cover" :alt="artistDetail.artist?.name" />
        </template>
      </div>
      ...
      <div class="flex-1">
        ...
        <template v-else>
          <h1 class="text-2xl font-bold mb-2">{{ artistDetail.artist?.name }}</h1>
          <div class="flex items-center text-sm text-gray-500 mb-4">
            <span v-if="artistDetail.artist?.alias && artistDetail.artist.alias.length > 0" class="mr-4">别名：{{ artistDetail.artist.alias.join('/') }}</span>
            <span>单曲数：{{ artistSongs.songs?.length || 0 }}</span>
            <span class="mx-2">|</span>
            <span>专辑数：{{ artistAlbums.hotAlbums?.length || 0 }}</span>
          </div>
          <div v-if="artistDetail.artist?.briefDesc" class="text-sm text-gray-600 dark:text-gray-400 mb-4 line-clamp-3">
            {{ artistDetail.artist.briefDesc }}
          </div>
          <div class="flex gap-3">
            <button class="px-4 py-2 bg-primary text-white rounded-full flex items-center" @click="playAllHotSongs()">
              <div class="i-carbon-play-filled mr-1"></div>
              <span>播放热门歌曲</span>
            </button>
            <button class="px-4 py-2 border border-gray-300 dark:border-gray-700 rounded-full flex items-center hover:bg-gray-100 dark:hover:bg-dark-800">
        ...
        <table class="w-full">
          ...
          <tbody>
            <template v-if="loading">
              <tr v-for="i in 10" :key="i" class="border-b border-gray-100 dark:border-gray-800">
                <td class="py-3 px-2">
            ...
            <template v-else-if="artistSongs.songs && artistSongs.songs.length > 0">
              <tr 
                ...
                <td class="py-3">
                  <div class="flex items-center">
                    ...
                    <div>
                      ...
                      <div v-if="song.alia && song.alia.length > 0" class="text-xs text-gray-500">{{ song.alia.join('/') }}</div>
                    </div>
                  </div>
                </td>
                <td class="py-3">
            ...
            <template v-else>
              <tr>
                <td colspan="4" class="py-8 text-center text-gray-500">暂无歌曲</td>
              </tr>
            </template>
          </tbody>
        </table>
    ...
    <div class="mb-8">
      <div class="flex items-center justify-between mb-4">
        ...
        <router-link :to="`/discover/album?artist=${$route.params.id}`" class="text-sm text-gray-500 hover:text-primary">更多 &gt;</router-link>
      </div>
      ...
      <div v-else class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
        ...
        >
          <div class="relative rounded-lg overflow-hidden aspect-square shadow-md mb-2">
            <img :src="album.picUrl + '?param=200y200'" class="w-full h-full object-cover" :alt="album.name" />
          </div>
          <div class="text-sm font-medium truncate">{{ album.name }}</div>
          <div class="text-xs text-gray-500">{{ formatDate(album.publishTime) }}</div>
        </div>
      </div>
    </div>
...
import { getArtistDetail, getArtistSongs, getArtistAlbums, getSimilarArtists } from '@/api/artist'
